...
- Column‑visibility checkboxes:
    - Drive Obsolescence (with only a few columns pre‑checked by default)
...
- Optional:
    ...
    - "Hide dominated combos" checkbox (in sidebar) based on combo‑level dominance.
- Reactor mass in combos is SCALED to the drive's power requirement.
- Mission feasibility search:
    - User inputs target Δv, accel (cruise/combat), and MINIMUM payload mass
    ...
    - Outputs a table with:
        ...
        - Result Delta-v (km/s)
        - Result Accel (g)
        - Max Feasible Payload (tons)  (analytic upper bound for this mission)
"""
...
<body>
  ...
  <main>
    ...
    <ul>
      ...
      <li>Ref Cruise Accel (g)</li>
      <li>Ref Combat Accel (g)</li>
    ...
    <p>
      When “care about crew size” is disabled, crew is ignored and cannot prevent
      a reactor from being dominated.
    </p>
    ...
    <p>
      By default, the plot shows <strong>Ref Cruise Accel (g)</strong> on the X-axis
    ...
    <ul>
      ...
      <li>Acceleration constraint:
        <ul>
          <li>Combat acceleration (g), or</li>
    ...
    <ul>
      ...
      <li>Result Accel (g)</li>
      <li>Max Feasible Payload (tons) – analytic upper bound for this mission</li>
    </ul>
  </main>
</body>
</html>
...
# Local HTML copies (must exist in same folder as this script)
LOCAL_DRIVE_HTML = "TIDriveTemplate.html"
LOCAL_PP_HTML = "TIPowerPlantTemplate.html"

# Default reference ship parameters for combined metrics
DEFAULT_REF_PAYLOAD_TONS = 1000.0
DEFAULT_REF_PROPELLANT_TONS = 1000.0  # e.g. 10 tanks of 100 t each

# Default fuel weights for Expensive Fuel Score
DEFAULT_FUEL_WEIGHTS = {
    "water": 1.0,
    "volatiles": 1.0,
    "metals": 1.0,       # base metals
    "nobleMetals": 1.0,
    "fissiles": 5.0,
    "antimatter": 20.0,
    "exotics": 20.0,
...
# Per‑tank propellant composition columns on drives
...
# Build‑material columns on power plants (not used yet, but handy to have)
...
def _extract_csv_from_html(html_text: str, header_prefix: str = "dataName,") -> str:
    """
    Extract the embedded CSV from a saved wiki HTML page.
    """
...
def _fetch_csv_from_local_html(local_html_path: str) -> pd.DataFrame:
    """
    Load CSV from a saved wiki HTML page (no online fetch).
    """
...
def _compute_drive_family_name(display_name: str) -> str:
    """
    Given a drive DisplayName like 'Tungsten Resistojet x3', return 'Tungsten Resistojet'.
    """
...
def load_drive_data() -> pd.DataFrame:
    ...
    # FamilyName groups variants like 'x1'..'x6' together
    df["FamilyName"] = df["DisplayName"].apply(_compute_drive_family_name)

    # Filter out disabled drives, if that column exists
...
def find_backup_power_column(df: pd.DataFrame) -> Optional[str]:
    """
    Try to detect which column (if any) contains backup power modes
    (Always, DriveIdle, DriveActive, Never).
    """
...
def find_drive_required_pp_column(drive_df: pd.DataFrame, pp_df: pd.DataFrame) -> Optional[str]:
    """
    Heuristically detect which column in the drive CSV corresponds to "Required Power Plant".
    """
...
def apply_profile(profile: Dict[str, Any]) -> None:
    """Apply a loaded profile into session_state."""
...
def sanitize_profile_dict(
    ...
    """
    Take an untrusted JSON object and return a safe, validated profile dict
    that matches the structure of build_profile_dict().
    """
    ...
    ignore_intraclass = to_bool(
        raw_profile.get("ignore_intraclass", False), False
    )

    # --- reference masses (respect UI ranges) ---
    ...
    # --- fuel weights (respect slider ranges) ---
...
def compute_drive_power_gw(thrust_n: float, ev_kps: float) -> float:
    """Approximate power usage (GW) from thrust and exhaust velocity."""
...
def drive_uses_scarce(row: pd.Series, abundance: Dict[str, bool]) -> bool:
    """
    True if this drive's per‑tank propellant uses any resource that is NOT abundant.
    """
...
def build_drive_features(
    ...
    for _, row in df.iterrows():
        ...
        # Propellant mix per tank & expensive fuel score
        mix_parts: List[str] = []
        exp_score = 0.0
        for res_key, col in DRIVE_PROP_RESOURCE_COLS.items():
            ...
            val = float(row[col])
            if val > 0:
                # wiki uses ×10 for display; scale here too
...
# ---------------------------------------------------------------------------
# Dominance / obsolescence logic for drives & reactors
# ---------------------------------------------------------------------------
...
def dominates_drive(
    ...
    """
    Return True if drive 'a' dominates drive 'b'.

    If ignore_intraclass is True and a[class_col] == b[class_col], then a
    is not allowed to dominate b (intra‑family dominance disabled).
    """
    if ignore_intraclass and (class_col in a.index) and (class_col in b.index):
        if a[class_col] == b[class_col]:
            return False

    # Scarcity: a drive that uses scarce propellant can never dominate
    # one that doesn't.
    ...
    if care_backup:
        # If b has idle backup and a does not, a cannot dominate b.
    ...
    # Improvement in scarcity dimension
...
def annotate_drive_obsolescence(
    ...
    obsolete_flags: List[bool] = [False] * len(feat_df)
    dominated_by: List[List[str]] = [[] for _ in range(len(feat_df))]
...
def dominates_pp(a: pd.Series, b: pd.Series, care_crew: bool) -> bool:
    ...
    Lower is better:
      ...
      - Crew (only if care_crew is True)
    """
    ge_dims = [
...
# ---------------------------------------------------------------------------
# Drive + Power Plant compatibility
# ---------------------------------------------------------------------------

def _normalize_class_name(s: Any) -> str:
    """
    Normalize class / requirement identifiers to a canonical form:

      "Solid_Core_Fission" -> "solid_core_fission"
      "Solid Core Fission" -> "solid_core_fission"
      "Any General"        -> "any_general"
    """
    if s is None:
        return ""
    s = str(s).strip()
    if not s:
        return ""
    # replace spaces with underscores, collapse multiple underscores, lowercase
    s = s.replace(" ", "_")
    s = re.sub(r"_+", "_", s)
    return s.lower()
...
def drive_compatible_with_pp(drive_row: pd.Series, pp_row: pd.Series) -> bool:
    """
    Basic compatibility: drive's Required Power Plant must match the reactor's Class,
    unless the drive says Any_General / Any Reactor / Any_* etc.

    We normalize both sides to handle underscores vs spaces consistently.
    """
    ...
    # No requirement → any reactor works
    if not req:
        return True
    ...
    # If reactor has no class, only wildcards can match (already handled above)
    if not plant:
        return False

    # "Any_<something>" pattern, e.g. "any_magnetic_confinement_fusion"
    if req.startswith("any_"):
        needed = req[len("any_"):]
        if not needed:
            return True
        # e.g. "magnetic_confinement_fusion" matches "toroid_magnetic_confinement_fusion"
        return (needed == plant) or (needed in plant)

    # Specific class requirement, e.g. "gas_core_fission", "solid_core_fission"
...
def build_valid_drive_pp_combos(
    ...
    """
    Build a table of valid (Drive, Power Plant) combinations, considering only
    non‑obsolete drives and non‑obsolete power plants.

    Adds combined metrics:
      - Reactor mass, dry mass, wet mass
      - Delta‑v (km/s) for ref_payload_tons & ref_propellant_tons
      - Cruise & combat acceleration (g)
      - Expensive Fuel Score
      - Power ratio & "enough power" flag

    IMPORTANT:
      - Reactor mass is now scaled to the drive's power requirement:
          PP Output Used (GW) = Drive Power (GW) (capped by PP Max Output)
          PP Reactor Mass (tons) = PP Output Used * Specific Power (tons/GW)
      - Combos where Drive Power > Max Output (GW) are flagged
        Reactor Enough Power? = False and filtered out later.
    """
    if drive_feat is None or pp_feat is None:
        return pd.DataFrame()

    # Only consider non‑obsolete drives/reactors
...
def combo_dominates(a: pd.Series, b: pd.Series) -> bool:
    """
    Return True if combo 'a' dominates combo 'b' based on ship-level metrics.
    ...
    """
    # Extract metrics, treating missing values as 0 for "ge" metrics
...
def annotate_combo_obsolescence(combos_df: pd.DataFrame) -> pd.DataFrame:
    """
    Annotate combo dataframe with combo-level obsolescence based on combo_dominates.
    """
    if combos_df.empty:
...
# ---------------------------------------------------------------------------
# Mission feasibility search (over payload & propellant)
# ---------------------------------------------------------------------------
...
def mission_feasibility_search(
    ...
    accel_type: str = "Combat",  # "Combat" or "Cruise"
    ...
    """
    For each combo, search over payload mass and propellant mass to see if
    there exists at least one (payload, propellant) pair that meets:

        - Δv >= dv_target_kps
        - accel (cruise or combat) >= accel_target_g

    Returns a table with one row per feasible combo, containing:
      - Drive
      - Power Plant
      - Payload Mass (tons)
      - Propellant Mass (tons)
      - Result Delta-v (km/s)
      - Result Accel (g)   (for the chosen accel_type)
      - Max Feasible Payload (tons) (analytic, for this mission target)

    The chosen (payload, propellant) pair for each combo is the feasible one
    with the smallest Total Wet Mass found on the grid.
    """
    if combos_df.empty:
        return pd.DataFrame()
    ...
    # Build search grids (linear spacing)
    if payload_steps < 2:
        payload_steps = 2
    if prop_steps < 2:
        prop_steps = 2
    ...
    for _, row in combos_df.iterrows():
        # Skip combos without enough power (should already be filtered, but be safe)
        ...
        m0 = drive_mass + reactor_mass  # tons (drive + reactor)

        # Effective thrust for the chosen accel metric
        t_eff = thrust * thrust_cap if use_combat else thrust
        if t_eff <= 0.0:
            continue

        # Analytic Max Feasible Payload (ignoring payload_min / grid limits)
        try:
        ...
        # Store (payload, propellant, total_wet, result_dv, result_accel)
        best_solution = None

        for Mp in payload_candidates:
            m_dry_base = m0 + Mp  # tons

            # Quick check: with max propellant, can we even reach dv_target?
            m_wet_max = m_dry_base + prop_max
            if m_wet_max <= m_dry_base:
                continue
            dv_max = ev_kps * math.log(m_wet_max / m_dry_base)
            if dv_max < dv_target_kps:
                # No point checking smaller prop masses; can't reach dv_target
                continue
...
# ---------------------------------------------------------------------------
# Slider <-> number_input sync callbacks
# ---------------------------------------------------------------------------
...
def main():
    ...
    st.markdown(
        textwrap.dedent(
            ...
            - Obsolescence respects **resource scarcity** and optional
            ...
            - Combined table and scatterplot at the bottom show only combos
              where the reactor has enough power, and can optionally hide
              dominated combos.
            """
        )
    )
    ...
    # Ensure numeric slider/input keys exist with defaults if not loaded
    ...
    # -----------------------------------------------------------------------
    # Sidebar: help, profile + global settings
    # -----------------------------------------------------------------------
    ...
    # Upload profile JSON (with size + schema validation)
    ...
    # Intraclass dominance toggle & combo hiding
    ...
    # Fuel weight sliders + inputs (above reference ship)
    ...
    # Reference ship mass sliders + inputs
    ...
    # -----------------------------------------------------------------------
    # Unlocked content management UI
    # -----------------------------------------------------------------------
    st.subheader("Unlocked Content")

    col_d, col_p = st.columns(2)

    # DRIVES SIDE (families)
    with col_d:
        st.markdown("### Drives (families)")

        # Reuse full list from earlier
    ...
    # -----------------------------------------------------------------------
    # Precompute drive & reactor feature tables (once per run)
    # -----------------------------------------------------------------------
    unlocked_drive_families = st.session_state.unlocked_drive_families
    ...
    # -----------------------------------------------------------------------
    # Analysis tabs: Drives & Power Plants
    # -----------------------------------------------------------------------
    ...
    with tab_drives:
        ...
        if drive_feat is None:
            ...
            base_cols = ["Name", "Obsolete", "Dominated By"]
            drive_property_cols = [c for c in drive_feat.columns if c not in base_cols]

            # Default: only these drive properties start selected
            ...
            with right_col:
                ...
                # Sort only on columns that actually exist
                ...
                # Force remount on structural changes (autosize columns)
    ...
    with tab_pp_tab:
        ...
        if pp_feat is None:
            ...
            with right_col:
                ...
                # Sort only on columns that exist
    ...
    # -----------------------------------------------------------------------
    # Combined Valid Drive + Power Plant section (below BOTH tabs)
    # -----------------------------------------------------------------------
    st.markdown("---")
    ...
    if (drive_feat is None) or (pp_feat is None):
        ...
        # Filter out combos where the reactor cannot power the drive
        if "Reactor Enough Power?" in combos_all.columns:
            combos_df = combos_all[combos_all["Reactor Enough Power?"]].copy()
        else:
            combos_df = combos_all.copy()
        ...
        if combos_df.empty:
            ...
            # Annotate combo-level dominance (used only for filtering, not displayed)
            combos_df = annotate_combo_obsolescence(combos_df)
            ...
            # Default: only these combo properties start selected
            ...
            # Use sidebar setting for hiding dominated combos
            ...
            if hide_dominated_flag and "Combo Obsolete" in combos_df.columns:
                combos_display = combos_df[~combos_df["Combo Obsolete"]].copy()
            else:
                combos_display = combos_df.copy()
            ...
            scatter_cols = {
                "Drive Expensive Fuel Score": "Drive Expensive Fuel Score",
                "Ref Delta-v (km/s)": "Ref Delta-v (km/s)",
                "Ref Cruise Accel (g)": "Ref Cruise Accel (g)",
                "Ref Combat Accel (g)": "Ref Combat Accel (g)",
                "Power Ratio (PP/Drive)": "Power Ratio (PP/Drive)",
                "Total Wet Mass (tons)": "Total Wet Mass (tons)",
            }

            # Only keep scatter options that actually exist in the dataframe
            available_scatter_cols = {
                label: col_name
                for label, col_name in scatter_cols.items()
                if col_name in combos_display.columns
            }
            ...
            if len(available_scatter_cols) < 2:
                ...
                labels = list(available_scatter_cols.keys())

                # Prefer Ref Cruise Accel (g) for X axis, Ref Delta-v (km/s) for Y axis
                if "Ref Cruise Accel (g)" in labels:
                    default_x_label = "Ref Cruise Accel (g)"
                else:
                    default_x_label = labels[0]

                if "Ref Delta-v (km/s)" in labels:
                    default_y_label = "Ref Delta-v (km/s)"
                else:
                ...
                # If X and Y are the same, show message and no chart
                if x_label == y_label:
                    st.info(
                    ...
                    x_col = available_scatter_cols[x_label]
                    y_col = available_scatter_cols[y_label]
            ...
            # Inputs: target Δv, accel constraint (type + value), and MIN payload
            ...
            if st.button("Calculate mission feasibility", key="btn_mission_feasibility"):
                if dv_target <= 0.0 or accel_target <= 0.0:
                    ...
                    # Use the same filtered combos the table/scatterplot are using
                    ...
                    if feas_df.empty:
                        ...
                        # Stable sort: alphabetical by drive then power plant
                        feas_sorted = feas_df.sort_values(
                            ["Drive", "Power Plant"]
                        ).reset_index(drop=True)

                        key_seed_feas = (
                            f"{feas_sorted.shape[0]}|{feas_sorted.shape[1]}"
                        )
                        df_key_feas = (
                            "df_mission_feas_"
